...
def load_exclusions(exclude_file: str) -> List[str]:
    ...
    try:
        ...
        if isinstance(content, dict):
            for key in ('exclusions', 'patterns'):
...
def organize_command(
    ...
    try:
        ...
        if not config.llm.api_key:
            console.print("❌ OpenAI API key not found. Please set the OPENAI_API_KEY environment variable or add it to your configuration.", style="red")
            return
        ...
        indexer = FileIndexer({"openai_api_key": config.llm.api_key, "model_name": config.llm.model_name})
        ...
        if not indexer.test_api_connection():
            ...
            console.print("Run 'llm-organizer test-api' for more detailed diagnostics.", style="yellow")
            return
        ...
        console.print("\nIf this is an API error, you can run 'llm-organizer test-api' to diagnose API connection issues.", style="yellow")
        raise
...
def test_api_command(config: Optional[AppConfig] = None) -> None:
    ...
    try:
        ...
        if not config.llm.api_key:
            ...
            console.print("Please set the OPENAI_API_KEY environment variable or add it to your configuration.", style="yellow")
            return
